...
public final class MessageController {
  ...
  public String viewAllMessages(@PathVariable("name") String topicName,
                                @Valid @ModelAttribute("messageForm") AllPartitionOffsetInfo messageForm,
                                BindingResult errors,
                                Model model) {
    final int size = (messageForm.getCount() != null ? messageForm.getCount().intValue() : 100);
    ...
    if (messageForm.isEmpty()) {
      final AllPartitionOffsetInfo defaultForm = new AllPartitionOffsetInfo();
    ...
    model.addAttribute("sortOrder", List.of(true, false));
    ...
    if(!messageForm.isEmpty() && !errors.hasErrors()) {
      ...
      if(! messageForm.getSortOrder())
        Collections.reverse(messages);
      model.addAttribute("messages", messages);
    }
    //return "topic-messages";
    return "allmessage-inspector";
  }
...
//Error at this place***************************************************
  ...
  List<Object> getPartitionOrMessages(
    ...
    if (partition == null || offset == null || count == null) {
      ...
      topic.getPartitions().forEach(vo -> partitionList.add(new PartitionOffsetInfo(vo.getId(), vo.getFirstOffset(),
        vo.getSize())));
      partitionList.add("All"); //Adding all option
  ...
  /**
   * Encapsulates offset data for a single topic.
   */
  public static class AllPartitionOffsetInfo {
    @NotNull
    @Min(1)
    @Max(500)
    ...
    private Boolean sortOrder = Boolean.TRUE; //True - Ascending; False - Descending

    public AllPartitionOffsetInfo(long offset,long count, MessageFormat format) {
      this.offset = offset;
      this.count = count;
      this.format = format;
    }

    public AllPartitionOffsetInfo(long offset, long count) {
      this(offset, count, MessageFormat.DEFAULT);
    }

    public AllPartitionOffsetInfo() {
